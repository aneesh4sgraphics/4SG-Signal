...
const US_NAMES = new Set(Object.values(US_STATE_ABBR_TO_NAME).map(n => n.toLowerCase()));
const CA_NAMES = new Set(Object.values(CA_PROVINCE_ABBR_TO_NAME).map(n => n.toLowerCase()));
...
function stateFilterVariants(canonical: string): string[] {
  // Strip any country suffix so "Alabama (US)" → "Alabama" before lookup
...
export function registerLeadsRoutes(app: Express): void {
  ...
  app.get("/api/leads/states", isAuthenticated, async (_req: any, res) => {
    try {
      const normalize = (raw: string): string => {
        ...
        // Append country suffix
        const lower = full.toLowerCase();
        if (US_NAMES.has(lower)) return `${full} (US)`;
        if (CA_NAMES.has(lower)) return `${full} (CA)`;
        return full;
      };
  ...
  app.post("/api/leads/:id/activities", isAuthenticated, async (req: any, res) => {
    try {
      ...
      // Update mailer timestamp fields on the lead
      if (data.activityType === 'mailer_one_page') {
        await db.update(leads).set({ onePageMailerSentAt: new Date(), lastMailerSentAt: new Date(), lastMailerType: 'one_page' }).where(eq(leads.id, leadId));
      } else if (data.activityType === 'mailer_envelope') {
        await db.update(leads).set({ sampleEnvelopeSentAt: new Date(), lastMailerSentAt: new Date(), lastMailerType: 'envelope' }).where(eq(leads.id, leadId));
      } else if (data.activityType === 'mailer_press_kit') {
        await db.update(leads).set({ pressTestKitSentAt: new Date(), lastMailerSentAt: new Date(), lastMailerType: 'press_kit' }).where(eq(leads.id, leadId));
      }
            ...
            if (anthropicKey) {
              try {
                ...
                const aiRes = await anthropic.messages.create({
                  ...
                  messages: [{
                    role: 'user',
                    content: `You are a sales assistant reading call notes. Extract structured follow-up info. Reply ONLY with JSON, no other text:
{
  "date": "YYYY-MM-DD or null",
  "intent": "brief action description or null",
  "promise": "what the rep promised to do (e.g. 'send quote by Friday', 'mail samples next week') or null",
  "priority": "high if urgent promise or hot lead, normal otherwise"
}

Today's date: ${new Date().toISOString().split('T')[0]}
Call notes: "${data.details.substring(0, 800)}"`
                  }]
                });
                ...
                if (jsonMatch) {
                  ...
                  if (parsedAI.date && parsedAI.date !== 'null') {
                    taskDueDate = new Date(parsedAI.date);
                    if (isNaN(taskDueDate.getTime())) taskDueDate = null;
                    else {
                      taskNote = parsedAI.promise || parsedAI.intent || taskNote;
                      taskType = parsedAI.promise ? 'follow_up' : 'call';
                    }
                  } else if (parsedAI.promise) {
                    // Promise found but no specific date — default to 2 days
                    taskDueDate = new Date();
                    taskDueDate.setDate(taskDueDate.getDate() + 2);
                    taskNote = parsedAI.promise;
                    taskType = 'follow_up';
                  }
                }
              } catch (aiError) {
          ...
          if (taskDueDate && !isNaN(taskDueDate.getTime())) {
            ...
            await storage.createFollowUpTask({
              ...
              taskType,
              priority: parsedAI?.priority === 'high' ? 'high' : 'normal',
              status: 'pending',
              dueDate: taskDueDate,
      ...
      // Auto-create follow-up task when a mailer is logged on a lead
      const mailerTypes = ['mailer_one_page', 'mailer_envelope', 'mailer_press_kit'];
      if (mailerTypes.includes(data.activityType)) {
        try {
          const daysUntilFollowUp = data.activityType === 'mailer_press_kit' ? 14
            : data.activityType === 'mailer_envelope' ? 10
            : 7;
          
          const dueDate = new Date();
          dueDate.setDate(dueDate.getDate() + daysUntilFollowUp);

          const mailerLabel = data.activityType === 'mailer_press_kit' ? 'Press test kit'
            : data.activityType === 'mailer_envelope' ? 'Sample envelope'
            : 'One-page mailer';

          const [lead] = await db.select({ name: leads.name, company: leads.company }).from(leads).where(eq(leads.id, leadId));
          
          await storage.createFollowUpTask({
            leadId,
            customerId: null,
            title: `${mailerLabel} sent — call to confirm receipt`,
            description: `${mailerLabel} was sent to ${lead?.company || lead?.name || 'lead'}. Call in ${daysUntilFollowUp} days to confirm they received it and ask for feedback.`,
            taskType: 'follow_up',
            priority: 'high',
            status: 'pending',
            dueDate,
            isAutoGenerated: true,
            sourceType: 'mailer_sent',
            sourceId: String(result[0].id),
            assignedTo: req.user?.id,
            assignedToName: req.user?.firstName ? `${req.user.firstName} ${req.user.lastName || ''}`.trim() : req.user?.email,
          });
        } catch (mailerTaskError) {
          console.warn('[Tasks] Failed to create mailer follow-up task:', mailerTaskError);
        }
      }
