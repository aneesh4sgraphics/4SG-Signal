...
export default function TaskInboxPage() {
  ...
  const navItems = [
    {
      ...
      items: [
        ...
        { key: "call_followups", label: "Call Follow-ups", count: summary?.callFollowUps, icon: Phone, countColor: (summary?.callFollowUps || 0) > 0 ? "bg-blue-100 text-blue-700" : "" },
      ],
    },
    {
      group: "Inbox",
  ...
  const sectionTitle: Record<string, string> = {
    ...
    overdue: "Overdue",
    email: "Email Tasks",
    call_followups: "Call Follow-ups",
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {task.sourceType === 'call_log' && (
                        <span className="flex-shrink-0 text-[10px] font-medium text-blue-600 bg-blue-50 px-1.5 py-0.5 rounded-full flex items-center gap-0.5 border border-blue-100">
                          <Phone className="h-2.5 w-2.5" />Call
                        </span>
                      )}
                    </div>
                    <div className="flex items-center gap-2 mt-0.5">
                      {(task.customerId || task.leadId) ? (
                        <Link href={task.recordType === 'lead' && task.leadId ? `/leads/${task.leadId}` : `/odoo-contacts/${task.customerId}`}
                          className="text-xs text-gray-500 truncate hover:text-violet-600 hover:underline"
                          onClick={e => e.stopPropagation()}>
                          {task.recordName || task.customerName}
                        </Link>
                      ) : (
                ...
                <div className="flex items-center gap-2">
                  {entry.customerId ? (
                    <Link href={`/odoo-contacts/${entry.customerId}`}
                      className="text-sm font-medium text-gray-800 truncate hover:text-violet-600 hover:underline">
                      {entry.contactName}
                    </Link>
                  ) : (
