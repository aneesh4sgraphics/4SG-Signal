...
- **Database Schema Optimization: min_order_qty Column (2025-07-23)**: Fixed critical database schema issue by converting min_order_qty from VARCHAR(50) to INTEGER with default value of 50. This prevents runtime parsing errors and improves data integrity. Updated all application code to work with integer values instead of string parsing. Schema now properly handles minimum order quantities as numeric values with appropriate defaults
