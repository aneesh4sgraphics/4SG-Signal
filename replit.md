...
- **Odoo V19 Enterprise Integration**: JSON-RPC API client for Odoo V19 Enterprise. Features customer sync (incremental and full reset), full address field mapping, bidirectional data access (products, pricelists, sale orders, users), bulk sync, and a guided product creation wizard for unmapped Odoo products. **Data quality rules**: Partners without email addresses are automatically skipped during import since email is required for CRM functionality. Blocked company types (cargo, freight, logistics, consolidators, 3PL, forwarding) are also skipped. **Customer exclusion list**: When an admin deletes a customer, their Odoo/Shopify IDs are recorded in `deleted_customer_exclusions` table to prevent re-import on future syncs. View exclusions at `/api/admin/customer-exclusions`. Email backfill endpoint available at `/api/odoo/backfill-emails`.
...
- **SPOTLIGHT Smart Hints**: Heuristics-based suggestions displayed above client cards to speed up processing. Hint types: (1) **Bad Fit detection** - flags 40+ non-printing business keywords (logistics, freight, shipping, consolidator, 3PL, real estate, legal, medical, restaurant, etc.) with one-click "Mark Bad Fit" action; (2) **Stale contact** - 180+ days no activity with "Send Reactivation Email" CTA; (3) **Duplicate detection** - normalized email/phone matching with "View to Merge", "Not a Duplicate", and "Skip" options; (4) **Already handled** - recent activity within 7 days with "Skip (Recent)" option; (5) **Quick win** - hot prospects flagged with "Call Now" priority; (6) **Missing field** - highlights when 2+ critical fields missing. Hints are color-coded by severity (high=red, medium=amber, low=gray) with dismissible CTAs. Key file: `server/spotlight-heuristics.ts`.
- **Do Not Merge Feature**: Users can mark customer pairs as separate entities via "Not a Duplicate" button in SPOTLIGHT duplicate hints. Schema: `customerDoNotMerge` table stores pairs with sorted IDs for consistent lookup. API: `POST /api/customers/do-not-merge`. Heuristics automatically exclude these pairs from future duplicate suggestions.
- **Admin Taxonomy Integration**: Machine types for customer profiles are fetched dynamically from `admin_machine_types` table via `/api/crm/machine-types` endpoint (authenticated users). CustomerCoachPanel.tsx uses this API to display machine profile options, with fallback to hardcoded defaults. Supports backward compatibility with legacy machine type codes in existing database records. Auto-seed on server startup via `server/taxonomy-seed.ts` ensures production deployments have required taxonomy data.
